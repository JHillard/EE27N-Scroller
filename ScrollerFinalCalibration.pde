...
Scroller scroller;
Robot mouse;
final int SCREEN_WIDTH = 600; //These refer to the 2 screens used for physical scroller
final int SCREEN_HEIGHT = 360;
ArrayList<ImageSet> imageSets;
int time = millis();
int wait = 2000;     //how long to wait at end of track for callibration.
int calibrate;    //calibrated length of track
Movie credits;
boolean setup = false;
...
void draw() {
  ...
  if(!setup) {
    ...
    lengthConst = (double)(scroller.getImageCount() - 1)/calibrate;
    setup = true;
...
void calibrateSliderDistance() {
    if ((millis()-time > wait) && (abs(position) > 1)){
      calibrate = abs(position);
      text ("Distance is: " + calibrate, 60, 120);
    }
    else {
      calibrate = 0;
    }
